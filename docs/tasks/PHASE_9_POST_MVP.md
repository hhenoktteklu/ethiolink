...
> **In progress.** Track 3 (Flutter mobile scaffold) — four commits landed: (1) "Phase 9: scaffold Flutter mobile app" — project structure, placeholder screens, `FakeAuthService`. (2) "Phase 9: add Flutter Cognito auth" — real `CognitoAuthService` via `flutter_appauth` + secure-storage token cache + on-near-expiry refresh + working sign-out. (3) "Phase 9: add mobile categories fetch" — Dio-backed `ApiClient` with an `AuthTokenInterceptor`, BrowseScreen wired to live `GET /v1/categories`. (4) "Phase 9: add mobile businesses listing" — tap a category card → `BusinessesScreen` powered by `GET /v1/businesses?category=<slug>`. `BusinessSummary` + `BusinessListPage` models, `HttpBusinessesRepository`, full loading / success / empty / error states, "Load more" cursor pagination. Featured-business chip surfaces `featuredUntil`. Next mobile commit: business detail screen + services / staff / reviews fetches as the booking-funnel anchor.
>

...
- [ ] Cash booking succeeds end-to-end; online booking attempt returns 400 with a clear message.<!-- `PaymentGateway` port + `CashGateway` (no-op SUCCEEDED) + `MockOnlineGateway` (throws typed `OnlinePaymentsUnavailableError` with code `ONLINE_PAYMENTS_UNAVAILABLE`) in place. `AppointmentService.create` routes by `paymentMethod` (CASH → cashGateway, ONLINE_PENDING → onlineGateway) and authorizes pre-INSERT. End-to-end wiring (handler + Lambda) is the next commit. -->

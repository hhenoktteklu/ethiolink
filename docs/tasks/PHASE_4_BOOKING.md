...
- [ ] Cash booking succeeds end-to-end; online booking attempt returns 400 with a clear message.<!-- `PaymentGateway` port + `CashGateway` (no-op SUCCEEDED) + `MockOnlineGateway` (throws typed `OnlinePaymentsUnavailableError` with code `ONLINE_PAYMENTS_UNAVAILABLE`) in place under `backend/shared/adapters/payments/`. Future Telebirr/Chapa/CBE_BIRR providers slot behind the same port without schema or domain changes. End-to-end wiring lands with the appointment service + handlers. -->

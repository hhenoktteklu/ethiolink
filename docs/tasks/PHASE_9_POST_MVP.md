...
- [ ] Flutter app ships with both `en` and `am` ARB bundles; locale picker in app settings.
  - [x] **Flutter i18n scaffold** (`Phase 9: add Flutter i18n scaffold`). `l10n.yaml` + `lib/l10n/app_en.arb` + `generate: true` in `pubspec.yaml` wire Flutter's `gen-l10n`. `MaterialApp` resolves `AppLocalizations.localizationsDelegates` + `AppLocalizations.supportedLocales`. A lightweight `LocaleScope` (`lib/core/i18n/locale_scope.dart`) publishes the active `Locale` to the `MaterialApp.locale` parameter — forward-prep for the picker commit. Visible English copy on login, bottom-nav, profile, bookings, owner dashboard, and the booking-flow confirm + success steps reads from `AppLocalizations.of(context)`. Strings are identical in English; nothing changes for end users until `app_am.arb` lands.
  - [ ] Amharic ARB bundle (`lib/l10n/app_am.arb`). Native-speaker review before prod rollout (see Risks section).
  - [ ] Locale picker in app Settings, mutating `LocaleScope` + PATCHing `users.locale` so the choice persists across devices.

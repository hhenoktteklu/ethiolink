...
>
> **Track 2 (Telegram bot provider) complete.** Five implementation commits closed the milestone. `2047637` (linking foundation: migrations 0014/0015, link-code repo + service, `GenericTelegramGateway`, `loadConfig` + `loadSecretsThenConfig` extensions), `25fdef3` (four Lambda endpoints + API GW routes + IAM scoping), `23eba88` (factory wiring + `pickNotificationChannel` priority `TELEGRAM > SMS > MOCK` across the 8 appointment handlers + reminder handler), `92218e1` (mobile `LinkTelegramScreen` + `HttpTelegramLinkRepository` reached from the Profile tab), and the operator runbook commit below. End-to-end on the mobile app, gated entirely on the env-stack flip: Profile → Telegram → Link → bot conversation opens → tap Start → app shows linked → next booking event delivers via Telegram with `notification_logs.channel = 'TELEGRAM'`. Six operator-led gates remain (see runbook): BotFather provisioning, Secrets Manager population, env-stack apply, `setWebhook` registration, mobile link smoke, end-to-end booking smoke. Until the operator flips `notifications_provider` to `"telegram"` or `"production"` in the env stack, the four Telegram endpoints return 503, the factory keeps the channel unmapped, and the dispatcher continues routing via SMS / MOCK exactly as before — zero production behaviour change from this track in unwired envs. Next workstream candidate: KMS encryption migration (Track 4) or localization / Amharic (Track 5).
>
...
- **Amharic / native localization.** Migration 0016 adds `users.locale` with default `'en'` + CHECK in `('en', 'am')`. `PATCH /v1/me` accepts `locale`. Template registry is keyed `template × locale`, with **English and Amharic entries for every booking template** as of the content pass. The registry still falls back to English when a renderer is missing, so adding a third locale stays safe. Date / time formatting uses a hand-rolled Amharic weekday + month abbreviation table + ጥዋት / ከሰዓት meridiem so unit tests don't depend on the Node ICU build. Flutter app ships with both ARB bundles (deferred). Admin SPA gets `react-i18next` scaffolding (lower priority — operator surface).
...
- `backend/shared/domains/notifications/templateRegistry.ts` *(widened — per-locale renderer table, `renderTemplate(key, payload, locale = 'en')`, transparent fallback to English when a locale-specific renderer is missing; Amharic renderers landed for every booking template as part of the content pass commit)*
...
- [x] Mobile `LinkTelegramScreen` (Profile-tab entry point on both customer + owner surfaces). *(Phase 9 commit "add Telegram mobile linking". New `mobile/lib/features/profile/link_telegram_screen.dart` + `data/telegram_link_repository.dart`. Five UI branches (loading / not-linked / linked / unconfigured-503 / error+retry); link flow opens deep link via `url_launcher` (test seam: injectable `LinkLauncher`) then polls status every 3 s for up to 90 s; manual "I linked it — check now" + Cancel buttons; idempotent Unlink. 10-case widget test suite + 7-case repository test suite, all platform-channel-free.)*
- [x] Operator runbook at `docs/operations/runbooks/telegram-bot.md`. *(Phase 9 commit "add Telegram bot runbook". Self-contained playbook: BotFather provisioning, the seven required Terraform variables (`notifications_provider` + bot-username + two secret ARNs + three optional knobs), both Secrets Manager shapes (plain + JSON with per-field selector + combined-secret option), Terraform apply walkthrough, `setWebhook` registration with `allowed_updates=["message"]` filter, the two smoke tests (mobile link + booking end-to-end with `notification_logs` SQL probe), three rollback paths (global disable / token rotation / full un-wire), seven troubleshooting cases (webhook 401, invalid token, chat-not-found, bot-blocked, 429, stuck polling, missing IAM), bot-token and webhook-secret rotation procedures, and six future-work items recorded.)*
...
- [x] Each consumer module accepts a `kms_key_id` input defaulting to `null` (= AWS-managed; no behavior change when unset). *(Phase 9 commit `Phase 9: wire CMKs through consumers`. RDS module gains `kms_key_id` + `secrets_kms_key_id`; S3 module gains `media_kms_key_arn` + `logs_kms_key_arn`; admin-frontend module gains `kms_key_arn`; secrets module gains `secrets_kms_key_arn`; lambda module gains `env_kms_key_arn` + `secrets_kms_key_arn` + `s3_media_kms_key_arn`. Every input defaults to `null` so existing AWS-managed encryption posture survives without any change. Setting an input flips the matching resource: RDS instance + master secret to CMK; S3 buckets to SSE-KMS with `bucket_key_enabled = true`; Lambda env-var blob to CMK.)*
- [x] Env stacks wire the new module's key ARNs to the consumer modules. *(Same commit. Both `environments/dev/main.tf` and `environments/prod/main.tf` thread `module.kms.<service>_key_arn` through every consumer module's new KMS inputs.)*
- [x] Per-domain Lambda IAM roles gain `kms:Decrypt` (+ `kms:GenerateDataKey*` for write paths) scoped to the relevant key ARNs. *(Same commit. New `lambda_kms_secrets` policy attached to every per-domain role (every domain reads the RDS master secret on cold start); new `lambda_kms_media` policy attached to the `media` role only (S3 GetObject + PutObject against SSE-KMS buckets). The SAR rotation Lambda gets a matching inline `kms:Decrypt` policy on its execution role. Every grant uses a `kms:ViaService` condition matching the `kms` module's key policy.)*
- [x] Re-encryption procedure documented in `docs/operations/runbooks/kms-migration.md`. *(Phase 9 commit `Phase 9: add KMS migration runbook`. Self-contained playbook: pre-flight checklist, RDS snapshot copy + restore + Option-A (`terraform import`) / Option-B (rename) cutover with a 10–20 minute read-only window, per-bucket S3 `aws s3 cp --recursive --metadata-directive REPLACE --sse aws:kms --sse-kms-key-id <arn>` (with the `--exclude "$(date +%Y-%m-%d)/*"` trick on the logs bucket), Secrets Manager forced rotation, Lambda env-var verification, end-to-end verification (resource-level + smoke + manual booking happy path), per-resource rollback, dev-first + prod-maintenance-window patterns, six recorded risks (RDS endpoint change vs. cached Lambda env; SSE-KMS cost; IAM `AccessDenied`; SAR rotation perms; pre-snapshot deletion timing; `aws s3 cp` mid-flight on the logs bucket), and a two-operator sign-off checklist. `SECURITY_REVIEW.md` finding #7 moved from "Deferred" to "Fixed (infra + docs); operator data-move pending".)*
- [ ] **Operator step** — Run the runbook end-to-end in dev, then prod. Until the dev pass completes, existing at-rest data continues encrypting under the AWS-managed keys; new writes after the wiring commit already encrypt under the CMK. Once dev passes, mark this checkbox + flip `SECURITY_REVIEW.md` finding #7 to "Fully fixed".
...
- [x] Template registry has `.am` variants for every customer-facing template. *(Content-pass commit — `Phase 9: add Amharic notification templates`. Amharic renderers cover all eight booking template keys. Date / time formatting falls back to a hand-rolled Amharic weekday + month abbreviation table + ጥዋት / ከሰዓት meridiem so the output is deterministic across CI runners regardless of the Node ICU build. Proper nouns — business name, service name, customer display name — pass through verbatim; we don't transliterate user-entered Latin-script data. Cancel-reason and reschedule-notes suffixes pick up Amharic labels (ምክንያት / ማስታወሻ). The English fallback still applies if a future locale lands without all its renderers.)*
- [x] Flutter app ships with both `en` and `am` ARB bundles; locale picker in app settings.
  - [x] **Flutter i18n scaffold** (`Phase 9: add Flutter i18n scaffold`). `l10n.yaml` + `lib/l10n/app_en.arb` + `generate: true` in `pubspec.yaml` wire Flutter's `gen-l10n`. `MaterialApp` resolves `AppLocalizations.localizationsDelegates` + `AppLocalizations.supportedLocales`. A lightweight `LocaleScope` (`lib/core/i18n/locale_scope.dart`) publishes the active `Locale` to the `MaterialApp.locale` parameter. Visible English copy on login, bottom-nav, profile, bookings, owner dashboard, and the booking-flow confirm + success steps reads from `AppLocalizations.of(context)`.
  - [x] **Amharic ARB bundle + locale picker** (`Phase 9: add Amharic mobile locale picker`). `lib/l10n/app_am.arb` mirrors every key in `app_en.arb` plus three picker-specific keys (`profileLanguageHeading`, `profileLanguageSaving`, `profileLanguageSaveError`). A `SupportedLocale` enum exposes the picker rows (English / አማርኛ — each in its own script so a user can identify the right row regardless of current UI language). Tapping a row drives `PATCH /v1/me { locale }` via `HttpMeRepository`, then flips `LocaleScope` + persists to `flutter_secure_storage` via `SecureLocalePreferences` so cold-starts respect the user's choice before the network round-trip. Failures surface a SnackBar with localized copy and leave the active locale untouched — the server-side `users.locale` row stays canonical. The notification dispatcher already honors `users.locale` (commits `ec9939d` + `747b1aa`), so flipping the picker yields Amharic SMS / Telegram bodies on the very next booking event.
  - [ ] **Native-speaker translation review.** Amharic copy was authored by the engineering team and should be vetted by a native-speaking reviewer before the Flutter app is promoted to TestFlight / Play Store rollout (see Risks section). The ARB key set is stable, so this is a string-only sweep.
- [ ] Admin SPA i18n scaffolding (lower priority — operator-facing surface, bilingual audience).

> Track 5 completion summary lives at [`docs/tasks/PHASE_9_LOCALIZATION_SUMMARY.md`](PHASE_9_LOCALIZATION_SUMMARY.md) — open operator gates (native-speaker review, TestFlight Amharic smoke, content-side Amharic population) and the deferred follow-ups are tracked there.

### Track 6 — Marketplace growth (independent commits)

- [x] Migration 0017 adds GIN index on `business_profiles` + full-text search column; `GET /v1/businesses` accepts new search filters. *(Phase 9 commit `Phase 9: add business full-text search`. Migration 0017 enables `pg_trgm` + `unaccent`, defines an `ethiolink_unaccent_immutable` SQL wrapper, adds a generated `search_tsv` column weighting `name` ('A') above `description.en` + `description.am` ('B'), and creates the `business_profiles_search_tsv_gin` + `business_profiles_name_trgm` indexes. `GET /v1/businesses` accepts `q` (new) with `query` kept as a deprecated alias, plus `featuredOnly` (boolean) and `sort` (enum: `featured` / `relevance` / `rating` / `newest`). Existing clients that only send `category` + `cursor` see no behavior change. `q` matches against name + description.en + description.am via `websearch_to_tsquery('simple', unaccent($))` complemented by a trigram `lower(name) ILIKE` fallback. Only `sort=featured` supports cursor pagination in this commit; the other three are first-page-only with `nextCursor: null`. `BusinessPublicView` gains an optional `searchRank` (non-null only under `sort=relevance` with non-empty `q`). Test suite extended with 14 new cases covering name / description.en / description.am matches, case + unaccent insensitivity, weight ordering (name above description), featuredOnly filtering, sort=rating + sort=newest orderings, and combined category+city+q.)*

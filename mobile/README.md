...
- ❌ Owner edit-business form, staff / availability / bookings owner screens. The "My Business" tab + the create-business + submit-for-review wizard + the services CRUD screen have landed (Track 3.5 in progress); the remaining four dashboard cards (Profile / Staff / Availability / Bookings) still SnackBar-stub. The next commit adds the staff CRUD screens — staff are the table availability is keyed on, so it's the binding prerequisite for the availability + bookings screens that follow.
...
**"Phase 9: add owner staff CRUD"** — replace the Staff dashboard-card SnackBar with a real screen mirroring the services CRUD shape. List → `GET /v1/businesses/{id}/staff`; Create → `POST /v1/businesses/{id}/staff` (name + role + active flag); Edit → `PATCH /v1/businesses/{id}/staff/{sid}`; Soft-delete → `DELETE /v1/businesses/{id}/staff/{sid}`. Reuses the same failure-kind classifier pattern + sheet-form pattern as the services screen. Staff are the binding prerequisite for the availability editor (availability rows are keyed on `staff_id`) and for the bookings screen (every appointment carries a `staff_id`). After staff lands the owner can stand up a complete roster and the customer-side booking flow's "Staff" step (which already exists in the customer mobile surface) starts working end-to-end against a real owner-managed roster.

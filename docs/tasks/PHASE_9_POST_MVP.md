...
> **In progress.** Track 3 (Flutter mobile scaffold) — seven commits landed: (1) "Phase 9: scaffold Flutter mobile app". (2) "Phase 9: add Flutter Cognito auth". (3) "Phase 9: add mobile categories fetch". (4) "Phase 9: add mobile businesses listing". (5) "Phase 9: add mobile business detail". (6) "Phase 9: add mobile booking flow". (7) "Phase 9: add mobile appointment history" — `BookingsScreen` lights up `GET /v1/me/appointments` grouped into Upcoming/Past, with `AppointmentDetailScreen` exposing the lifecycle actions: Cancel while REQUESTED/ACCEPTED (409 CONFLICT → "Past the cancellation cutoff"), Review while COMPLETED (1–5 stars + optional comment; 409 CONFLICT → "Already reviewed"). **The customer-facing MVP loop is now end-to-end on mobile** — browse → category → business → service → slot → confirm → see in history → cancel before cutoff OR review after completion. Next mobile commit: customer-surface completion summary capturing the seven commits + Track 3 acceptance state.
>

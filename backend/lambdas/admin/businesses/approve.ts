// EthioLink — Lambda handler for `POST /v1/admin/businesses/{id}/approve`.
//
// Authenticated, ADMIN-only. Transitions a `PENDING_REVIEW` business
// to `APPROVED` and records one `APPROVE_BUSINESS` row in
// `admin_actions`. Body accepts an optional `{ notes }` field; if
// provided, the notes land in the audit row.
//
...
export const handler = async (
    ...
    const logger = baseLogger.child({
        ...
        handler: 'admin.businesses.approve',
    });
    ...
    try {
        ...
        try {
            const updated = await adminBusinessService.approveBusiness(
                id,
                authz.caller,
                notes,
            );
        ...
        logger.error('admin.businesses.approve.failed', {

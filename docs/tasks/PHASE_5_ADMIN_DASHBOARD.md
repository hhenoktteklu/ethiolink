...
- [ ] All admin write endpoints persist an `admin_actions` row.<!-- Repository in place: `backend/shared/domains/admin/adminActionRepository.ts` exposes `insert` + `listByAdmin` + `listForTarget` only (append-only, no `update` / `delete`). `AdminAction` and `AdminTargetType` unions are application-layer enums — additive contract; no DB CHECK. View module (`adminActionView.ts`) returns JSON with ISO-8601 timestamps. First consumer wired: `AdminBusinessService` (approve / reject / suspend / setFeaturedUntil) records one audit row per success, none on failure. Remaining consumers (user, category) land next; the Lambda handler wiring lands after the services. -->

...
> **In progress.** Track 3 (Flutter mobile scaffold) — six commits landed: (1) "Phase 9: scaffold Flutter mobile app" — project structure, placeholder screens, `FakeAuthService`. (2) "Phase 9: add Flutter Cognito auth" — real `CognitoAuthService` via `flutter_appauth` + secure-storage token cache + on-near-expiry refresh + working sign-out. (3) "Phase 9: add mobile categories fetch" — Dio-backed `ApiClient` with an `AuthTokenInterceptor`, BrowseScreen wired to live `GET /v1/categories`. (4) "Phase 9: add mobile businesses listing" — `BusinessesScreen` per category with `BusinessSummary` model + cursor pagination. (5) "Phase 9: add mobile business detail" — composes four concurrent fetches into one scrollable page. (6) "Phase 9: add mobile booking flow" — `BookingFlowScreen` wizard (staff → date → slot → confirm → success) over `GET /slots` + `POST /v1/appointments`. **The mobile app's first authenticated write is now wired end-to-end.** Error branching covers `SLOT_UNAVAILABLE` (one-tap return to the slot step), `UNAUTHENTICATED` (sign-in copy), and network / 5xx (generic retry). Next mobile commit: bookings tab fetches `GET /v1/me/appointments`, closing the customer-facing MVP loop (browse → book → history → review).
>
...
- [ ] Customer browse + book + history flows light up against the dev API. *(Browse + book paths complete — Phase 9 commits "add mobile categories fetch" + "add mobile businesses listing" + "add mobile business detail" + "add mobile booking flow". History fetch is the remaining sub-item.)*

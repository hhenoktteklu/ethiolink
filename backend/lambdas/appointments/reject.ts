// EthioLink — Lambda handler for `POST /v1/appointments/{id}/reject`.
//
// Authenticated. Business owner (or ADMIN) refuses a REQUESTED
// appointment, transitioning it to REJECTED. Ownership / admin
// override is enforced by `AppointmentService.reject`.
//
// Body (optional): `{ "reason": "string?" }`. Parsed and validated
// here for forward compatibility, but NOT persisted in MVP — the
// `appointments` schema has no `reject_reason` column. The reason
// is logged at INFO level so it's recoverable for support inquiries
// without a schema change. When a real "reject reason" requirement
// emerges, the column lands in a new migration and the service grows
// a parameter.
//
...
export const handler = async (
    ...
    try {
        ...
        try {
            const appointment = await appointmentService.reject(id, {
                userId: user.id,
                role: principal.role,
            });
            // Reject reasons are logged in MVP (no schema column yet).
            if (reason !== null) {
                logger.info('appointments.reject.reason', {
                    appointmentId: id,
                    reason,
                });
            }
        ...
        logger.error('appointments.reject.failed', {

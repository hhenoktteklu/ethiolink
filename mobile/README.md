...
- ❌ Owner edit-business form, owner bookings inbox, open-date availability overrides. The "My Business" tab + create-business + submit-for-review wizard + services CRUD + staff CRUD + availability editor screens have landed (Track 3.5 in progress); the remaining two dashboard cards (Profile / Bookings) still SnackBar-stub. The next commit adds the bookings inbox — accept / reject / mark-complete on per-business `GET /v1/businesses/{id}/appointments`.
...
**"Phase 9: add owner bookings inbox"** — replace the Bookings dashboard-card SnackBar with the real owner-side appointments inbox. List → `GET /v1/businesses/{id}/appointments` with status + date filters; tap a row → action sheet with the lifecycle transitions: ACCEPT (`POST /v1/appointments/{id}/accept`), REJECT (`POST /v1/appointments/{id}/reject`), MARK COMPLETE (`POST /v1/appointments/{id}/complete`), NO-SHOW (`POST /v1/appointments/{id}/no-show`). Reuses the failure-kind classifier pattern. Once it lands, the owner-mobile MVP loop is fully end-to-end: create business → submit → admin approves → add services + staff + availability → customer books → owner accepts → service performed → owner marks complete → customer reviews.

...
- ❌ Owner edit-business form, availability / bookings owner screens. The "My Business" tab + create-business + submit-for-review wizard + services CRUD + staff CRUD screens have landed (Track 3.5 in progress); the remaining three dashboard cards (Profile / Availability / Bookings) still SnackBar-stub. The next commit adds the availability editor — availability rows are keyed on `staff_id` (now real) and the owner-side bookings inbox depends on staff being addressable.
...
**"Phase 9: add owner availability editor"** — replace the Availability dashboard-card SnackBar with a real screen. Per-staff weekly schedule editor backed by `GET /v1/businesses/{id}/staff/{sid}/availability` + `PUT` (replace the full week as one transaction). Day-by-day row UI: each weekday has an "add interval" affordance and the existing intervals as `start–end` pickers; saving issues a single PUT with the full week, mirroring the backend's "replace the whole week" semantics. Date-overrides (`POST /v1/businesses/{id}/staff/{sid}/availability/override`) land as a follow-up — this commit ships the weekly editor only, which is enough for the customer-side slot picker to compute real slots against an owner-curated schedule. After availability lands, the bookings inbox is the last owner-flow gate before the owner-mobile MVP loop is fully end-to-end.

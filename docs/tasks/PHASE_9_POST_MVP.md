...
> **In progress.** Track 1 (real SMS provider) — gateway, factory, secret resolution, and IAM scoping shipped across two commits. (1) "Phase 9: add SMS provider gateway" — gateway + config types + tests. (2) "Phase 9: wire SMS provider into dispatcher" — `loadSecretsThenConfig` resolves `SMS_PROVIDER_API_KEY_SECRET_ARN`, `notificationServiceFactory` selects between `MockNotificationGateway` and `GenericSmsGateway` based on `config.smsProvider` + `NOTIFICATIONS_PROVIDER`, 9 handlers refactored onto the factory, Lambda IAM grants the SMS-secret read only to `appointments`+`scheduled` roles. **Production traffic still routes through MOCK** until the operator (a) sets the env vars + secret ARN in the env stacks, (b) flips `notifications_provider` to `sms`, and (c) ships the small follow-up that switches `appointmentService.notify`'s hardcoded `channel: 'MOCK'` to a per-recipient channel selection.
...
- [ ] `mobile/` directory scaffolded; `flutter analyze` + `flutter test` clean.
- [ ] `flutter_appauth` PKCE wiring against the Cognito mobile app-client; deep-link callback on `ethiolink://auth/callback`.
- [ ] OpenAPI-generated Dart client lands at `mobile/lib/api/generated/` and rebuilds on every API change.

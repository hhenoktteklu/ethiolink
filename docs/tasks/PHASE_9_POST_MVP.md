...
> **In progress.** Track 3 (Flutter mobile scaffold) — five commits landed: (1) "Phase 9: scaffold Flutter mobile app" — project structure, placeholder screens, `FakeAuthService`. (2) "Phase 9: add Flutter Cognito auth" — real `CognitoAuthService` via `flutter_appauth` + secure-storage token cache + on-near-expiry refresh + working sign-out. (3) "Phase 9: add mobile categories fetch" — Dio-backed `ApiClient` with an `AuthTokenInterceptor`, BrowseScreen wired to live `GET /v1/categories`. (4) "Phase 9: add mobile businesses listing" — `BusinessesScreen` per category with `BusinessSummary` model + cursor pagination. (5) "Phase 9: add mobile business detail" — tap a business row → `BusinessDetailScreen` composing four concurrent fetches (business / services / staff / reviews) into one scrollable page. Each section renders its own sub-state so a 5xx on one doesn't blank the others. `Service`, `Staff`, `Review`, `BusinessDetail` models; `Http{BusinessDetail,Services,Staff,Reviews}Repository` ports bundled into `BusinessDetailRepositories`. Next mobile commit: slot picker + appointment creation — the booking funnel's anchor screens and the mobile app's first authenticated write.
>
...
- [ ] Customer browse + book + history flows light up against the dev API. *(Browse path complete — categories grid + per-category business listing wired in Phase 9 commits "add mobile categories fetch" + "add mobile businesses listing". Booking funnel + customer-history fetches are the remaining sub-items.)*

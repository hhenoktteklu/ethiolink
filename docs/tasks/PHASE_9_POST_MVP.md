...
> **In progress.** Track 3 (Flutter mobile scaffold) — initial commit "Phase 9: scaffold Flutter mobile app" lands the project structure (`pubspec.yaml`, `analysis_options.yaml`, `.gitignore`, `lib/core/{config,api,auth}/`, `lib/features/{auth,browse,bookings,profile}/`, a widget smoke test, and a `README.md` setup playbook). The scaffold runs end-to-end with a `FakeAuthService` placeholder; real Cognito PKCE wiring is the next mobile commit. See `mobile/README.md` for the local-run procedure.
>
...
- [ ] `flutter_appauth` PKCE wiring against the Cognito mobile app-client; deep-link callback on `ethiolink://auth/callback`. *(Next mobile commit — `AuthService` port already exists; only the implementation swaps.)*

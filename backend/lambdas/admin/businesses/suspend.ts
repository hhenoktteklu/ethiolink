// EthioLink — Lambda handler for `POST /v1/admin/businesses/{id}/suspend`.
//
// Authenticated, ADMIN-only. Transitions an `APPROVED` or
// `PENDING_REVIEW` business to `SUSPENDED` and records one
// `SUSPEND_BUSINESS` row in `admin_actions`. Body accepts optional
// `{ notes }` (rendered alongside the business in the dashboard).
//
...
export const handler = async (
    ...
    const logger = baseLogger.child({
        ...
        handler: 'admin.businesses.suspend',
    });
    ...
    try {
        ...
        try {
            const updated = await adminBusinessService.suspendBusiness(
                id,
                authz.caller,
                notes,
            );
        ...
        logger.error('admin.businesses.suspend.failed', {

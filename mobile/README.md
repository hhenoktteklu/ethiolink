...
4. **Apply the iOS overrides** (iOS targets only — skip on Android-only setups).
   ```bash
   bash scripts/configure_ios.sh
   cd ios && pod install && cd ..
   ```
   Sets the bundle id + Cognito URL scheme + iOS-13 floor on the freshly-generated `ios/` tree. Idempotent — re-run after any future `flutter create .`. See the "iOS — `scripts/configure_ios.sh`" section below for what it changes.

5. **Run the app.**
   ```bash
   # Android emulator (device id from `flutter devices`):
   flutter run -d emulator-5554 --dart-define-from-file=env/dev.json

   # iOS simulator (device name from `flutter devices`):
   flutter run -d "iPhone 17" --dart-define-from-file=env/dev.json
   ```
   Or against any other env file (`env/staging.json`, `env/prod.json`) when those land. Plain `flutter run --dart-define-from-file=env/dev.json` targets whichever single device is attached.
...
### iOS — `scripts/configure_ios.sh`

The iOS platform tree (`ios/`) is gitignored and regenerated by `flutter create .`, so the EthioLink-specific overrides are applied by an idempotent post-generate script rather than committed verbatim (the iOS analogue of the committed Android `appAuthRedirectScheme` gradle override). After `flutter create .`, run:
...
`configure_ios.sh` does three things, all idempotent (safe to re-run after any `flutter create .` / `flutter clean`):

1. **Bundle identifier** → `com.ethiolink.app` on the Runner target (RunnerTests keeps its `.RunnerTests` suffix). Matches the Android `applicationId` and the reverse-domain Cognito redirect scheme.
2. **`Info.plist` `CFBundleURLTypes`** → registers the `com.ethiolink.app` URL scheme so iOS routes the Cognito redirect (`com.ethiolink.app:/oauthredirect`) and logout (`com.ethiolink.app:/logout`) back into the app. The resulting plist entry is:
...
3. **Podfile iOS deployment-target floor** → `platform :ios, '13.0'`. `flutter_appauth ^12.0.0` requires iOS 13 minimum; modern Flutter scaffolds already default there, but the script pins it explicitly.
...
No ASWebAuthenticationSession entitlement or extra capability is needed — `flutter_appauth` on iOS 13+ drives the system browser via the AppAuth-iOS SDK with only the URL scheme above. Signing / provisioning stays in Xcode and is developer-account specific; the script doesn't touch it.
...
- ✅ `ApiClient` over Dio with an `AuthTokenInterceptor` — attaches `Authorization: <idToken>` (bare Cognito ID token, no `Bearer ` prefix; the backend API Gateway REST `COGNITO_USER_POOLS` authorizer validates the raw header value as a JWT and rejects `Bearer eyJ…` with 401) when a session exists; public endpoints work without one. One-shot 401 retry after a token refresh.
...
- ❌ Per-platform scaffolding (`ios/`, `web/`, `linux/`, `macos/`, `windows/`) — regenerated by `flutter create .`. The Android scaffold IS committed (it carries the `appAuthRedirectScheme` gradle override). iOS overrides (bundle id `com.ethiolink.app` + the `CFBundleURLSchemes` entry + the iOS-13 Podfile floor) are applied by the committed `scripts/configure_ios.sh` after `flutter create .` rather than committed verbatim, since the generated `ios/` tree is gitignored.

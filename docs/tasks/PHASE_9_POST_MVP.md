...
> **In progress.** Track 3 (Flutter mobile scaffold) — two commits landed: (1) "Phase 9: scaffold Flutter mobile app" — project structure, placeholder screens, `FakeAuthService`. (2) "Phase 9: add Flutter Cognito auth" — real `CognitoAuthService` via `flutter_appauth` + secure-storage token cache + on-near-expiry refresh + working sign-out, swapped into `LoginScreen` with `FakeAuthService` retained for tests. Platform deep-link setup (Android intent filter + iOS URL scheme) documented in `mobile/README.md`. Next mobile commit: Dio + auth-token interceptor + first real `GET /v1/categories` fetch.
>
